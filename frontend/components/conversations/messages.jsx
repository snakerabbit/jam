...
class Messages extends React.Component{
  ...
  if(Object.keys(this.props.currentConversation.messages).length !== Object.keys(newProps.currentConversation.messages).length){
    this.props.fetchConversation(this.props.conversationId);
  }
  ...
  handleSubmit(e){
    ...
    this.props.createMessage(newMessage).then(()=> this.setState({messageText:""}));
  }
    ...
    if(this.props.currentConversation) {
      return (
        Object.keys(this.props.currentConversation.messages).map((message) => {
          ...
          return(
            <div key={message} className='chat-bubble' id='out'>
  ...
  render(){
      return(
        ...
        <h1 className='message-board-other'>{this.otherUserName()}</h1>
